...
class Model_Section extends \Orm\Model {
	...
	public function generate_fieldset()
	{
		...
		if (\Arr::get($this->_fieldset_data, 'survey_template', true))
		{
			...
			$fieldset->form()->set_config('multi_field_template', "<div class=\"question\"><div class=\"{error_class} question-title\">{group_label}{required}</div><div class=\"{error_class} answer\">{fields}<div class=\"survey-input\">{field} {label}</div>{fields}{error_msg}</div></div>\n");

			$fieldset->form()->set_config('field_template', "\t\t<div class=\"question\">\n\t\t\t<div class=\"{error_class} question-title\">{label}{required}</div>\n\t\t\t<div class=\"{error_class} answer\"><div class=\"survey-input\">{field}</div> {error_msg}</div>\n\t\t</div>\n");
		}
		...
		if ($fieldset->validation()->run())
		{
			...
			if ($fieldset->validation()->validated('submit-'.$this->id))
			{
				...
				foreach ($fieldset->field() as $key=>$field)
				{
					...
					$session[$this->id][$qid] = ($fieldset->validation()->input($key).'');
				}
